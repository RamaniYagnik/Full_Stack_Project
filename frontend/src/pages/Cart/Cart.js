                                ...
                                <div key={idx} className='flex flex-col sm:flex-row bg-white rounded-xl shadow-md p-4'>
                                    ...
                                    <div className='pl-4 flex flex-col justify-between flex-1'>
                                        <div className='max-w-full'>
                                            ...
                                            <h3 className='text-lg font-semibold md:line-clamp-3 line-clamp-1'>{item.product?.productName?.split(' ').slice(0, 5).join(' ') + (item.product?.productName?.split(' ').length > 5 ? '...' : '')}</h3>
                                        </div>
                                        <div className='flex items-center mt-2'>
                                            <button onClick={() => decreaseQyt(item._id, item.quantity)} className='px-2 py-1 border rounded hover:bg-yellow-600 hover:text-white transition-all'>-</button>
                                            <span className='px-4'>{item.quantity}</span>
                                            <button onClick={() => increaseQyt(item._id, item.quantity)} className='px-2 py-1 border rounded hover:bg-yellow-600 hover:text-white transition-all'>+</button>
                                        </div>
                                    </div>
                                    <div className='sm:ml-auto flex flex-col justify-between items-end text-right sm:h-full sm:justify-between sm:py-2'>
                                        <button className='group p-2 hover:text-white hover:bg-yellow-600 text-xl hover:border-yellow-600 border border-slate-400 transition-all rounded-lg' onClick={() => deleteProduct(item?._id)} ><IoIosTrash className='transition-all duration-800 group-hover:scale-150' /></button>
